...
namespace Augments.Projectiles
{
    public class MediGunBeamProjectile : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            // 3. Aim player and projectile position
            Vector2 aimTarget = targetPlayerWhoAmI >= 0
                ? Main.player[targetPlayerWhoAmI].Center
                : (targetNPCWhoAmI >= 0 ? Main.npc[targetNPCWhoAmI].Center : Main.MouseWorld);
            ...
            if (isLocked)
            {
                ...
                if (healPulseTimer >= HealPulseInterval)
                {
                    ...
                    if (targetPlayerWhoAmI >= 0)
                    {
                        ...
                        SoundEngine.PlaySound(SoundID.Item29 with { Volume = 0.5f, Pitch = 0.4f }, target.Center);
                    }
                    else if (targetNPCWhoAmI >= 0)
                    {
                        ...
                        SoundEngine.PlaySound(SoundID.Item29 with { Volume = 0.5f, Pitch = 0.4f }, npc.Center);
                    }
                }
            }
            else
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Vector2 targetPos = targetPlayerWhoAmI >= 0
                ? Main.player[targetPlayerWhoAmI].Center
                : (targetNPCWhoAmI >= 0 ? Main.npc[targetNPCWhoAmI].Center : (muzzlePos + (Main.MouseWorld - muzzlePos).SafeNormalize(Vector2.UnitX) * Math.Min(MaxAcquireRange, Vector2.Distance(muzzlePos, Main.MouseWorld))));
            ...
            for (int i = 1; i <= segments; i++)
            {
                ...
                Color outerColor = (targetPlayerWhoAmI >= 0 || targetNPCWhoAmI >= 0)
                ...
                // Inner bright core
                Color innerColor = (targetPlayerWhoAmI >= 0 || targetNPCWhoAmI >= 0)
